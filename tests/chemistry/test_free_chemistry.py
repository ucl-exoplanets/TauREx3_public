...
@given(ratio=floats(min_value=0.0, max_value=0.9))
@settings(deadline=None)
def test_fill_gas_pair_ratio(ratio):
    ...
    tc.initialize_chemistry(1, np.array([1000]), np.array([1e6]))
...
@given(mols=lists(molecule_vmr(), min_size=1))
@settings(deadline=None)
def test_constant_profile(mols):
    ...
    if sum([v for s, v in successful]) > 1.0:
        with pytest.raises(InvalidModelException):
            tc.initialize_chemistry(1, np.ones(1)*1000, np.ones(1)*1e5)
        return
    else:
        tc.initialize_chemistry(1, np.ones(1)*1000, np.ones(1)*1e5)
