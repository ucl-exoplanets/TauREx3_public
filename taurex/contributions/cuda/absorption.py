...
import numpy as np
from taurex.contributions.absorption import AbsorptionContribution
from numba import cuda

@cuda.jit
def absorption_cuda(sigma,density,path,nlayers,ngrid,nmols,layer,tau):
...
class GPUAbsorptionContribution(AbsorptionContribution):
    ...
    def contribute(self,model,layer,density,path_length,return_contrib):
        ...
        absorption_cuda[self._griddim,self._blockdim,self._stream](self._device_sigma,self._device_dens,self._dpath[layer],self._nlayers,self._ngrid,self._nmols,layer,self._device_tau)
    ...
    def prepare(self,model,wngrid):
        ...
        self._device_dens = cuda.to_device(model.densityProfile)
        self._device_sigma = cuda.to_device(self.sigma_xsec,stream=self._stream)
