...
class PhoenixStar(BlackbodyStar):
    ...
    def recompute_spectra(self):
        ...
        if self.temperature > self._T_list.max() or self.temperature < self._T_list.min():
    ...
    def get_avail_phoenix(self):
        ...
        self._files = files
        self._T_list    =  np.array([np.float(os.path.basename(k)[3:8])   for k in files])*100
        self._Logg_list =  np.array([np.float(os.path.basename(k)[9:12])  for k in files])
        self._Z_list    =  np.array([np.float(os.path.basename(k)[13:16]) for k in files])
        self._index_finder = NearestNDInterpolator((self._T_list,self._Logg_list,self._Z_list),np.arange(0,self._T_list.shape[0]))        
