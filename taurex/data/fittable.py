...
def fitparam(f=None,param_name=None,param_latex=None,default_mode='linear',default_fit=False,default_bounds=None):
    """
    A decorator used in conjunction with :class:`Fittable` to inform which parameters can be fit and
    its properties. On its own it acts like the ``property`` decorator.
    When used within a :class:`Fittable` class it serves to tag a property as able to fit and allows the
    ...
    default_mode : ``linear`` or ``log``
        Defines how the optimizer should read and write the parameter. ``linear`` reads/write everything as is.
        ``log`` informs the optimizer to transform from native->log space when read and to transfrom log->native when
    ...
    default_bounds : :obj:`list`
        Default minimum and maximum fitting boundary. Must always be defined in the native space
...
class Fittable(object):
    ...
    class with the ability to read and write fitting parameters using their params names, for example,
    ...
    def add_fittable_param(self,param_name,param_latex,fget,fset,default_mode,default_fit,default_bounds):
        """
        Adds a fittable parameter to the internal dictionary. Used during init to add all :func:`fitparam`
        decorated methods and can also be utilized by a user to manually add new fitting parameters. This is
        useful for giving fitting parameters names that depend on certain attributes 
        (e.g. molecule name in a gas profile see :class:`~taurex.data.profiles.chemistry.gas.constantgas.ConstantGas`)
        or when converting lists into fitting parameters 
        (e.g. Normalization factor in light curves see: :class:`~taurex.model.lightcurve.lightcurve.LightCurveModel` )
    ...
    def compile_fitparams(self):
        """
        Loops through and finds all fitting parameters in the class and adds it to the internal
        dictionary
        """
