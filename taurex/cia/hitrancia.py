...
class HitranCIA(CIA):
    ...
    A class that directly deals with HITRAN `cia <https://hitran.org/cia/>`_ files
    and turns them into generic CIA objects that nicely produces cross sections for us.
    This will handle CIAs that have wavenumber grids split across temperatures by unifying
    ...
    def compute_final_grid(self):
        ...
        and unifies them into a single temperature, cross-section and wavenumber grid
        for us to FINALLY interpolate and produce collisionaly induced cross-sections
